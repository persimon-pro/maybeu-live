...
const QuizControl: React.FC<Props> = ({ activeEvent, lang }) => {
  // Загружаем вопросы из activeEvent.questions (теперь они там)
  ...
  // При загрузке компонента или смене события, обновляем локальный стейт вопросов
  useEffect(() => {
    setQuestions(activeEvent.questions || []);
  }, [activeEvent]);

  // Функция для сохранения вопросов в Firebase
  const saveQuestionsToFirebase = (newQuestions: QuizQuestion[]) => {
    setQuestions(newQuestions); // Обновляем локально
    
    // Обновляем весь объект события в Firebase
  ...
  const handleGenerateQuestions = async () => {
    ...
    saveQuestionsToFirebase(updatedList); // Сохраняем в облако
  ...
  const handleSaveEdit = () => {
    ...
    saveQuestionsToFirebase(updated); // Сохраняем в облако
    setEditingIdx(null);
  };
  ...
  const removeQuestion = (idx: number) => {
    ...
    saveQuestionsToFirebase(updatedList); // Сохраняем удаление
