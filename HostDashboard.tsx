...
const HostDashboard: React.FC<Props> = ({ activeEvent, setActiveEvent, lang }) => {
  ...
  const [events, setEvents] = useState<LiveEvent[]>([]); // Данные теперь придут из Firebase
  ...
  // Если активное событие обновилось (например, мы его отредактировали), сохраняем в базу
  useEffect(() => {
    if (activeEvent) {
       // Это сохранит любые изменения (включая сценарий и заметки) в облако
       FirebaseService.saveEventToDB(activeEvent);
    }
  }, [activeEvent]); 

  // Мониторинг экрана (Heartbeat)
  useEffect(() => {
  ...
  const handleSaveEvent = () => {
    if (editingEvent) {
      ...
      const event: LiveEvent = {
        ...
        status: 'UPCOMING',
        timetable: [],
        questions: [] // Инициализируем массив вопросов
      };
  ...
  const handleToggleLive = () => {
    ...
    // Синхронизируем статус и данные
    FirebaseService.syncEvent(updated);

    if (isStopping) {
      // При завершении включаем форму лидов
  ...
  const updateDetail = (field: keyof LiveEvent, value: any) => {
    ...
    // Сохранение в Firebase произойдет автоматически через useEffect
  };
  ...
  return (
    <div className="flex flex-col h-full bg-slate-950 overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto p-4 md:p-8">
        ...
        {/* ... Остальной код табов INFO и TIMING без изменений, так как они используют updateDetail, который мы уже переписали для Firebase ... */}
