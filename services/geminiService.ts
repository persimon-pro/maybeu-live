...
// --- ЕДИНСТВЕННОЕ ИЗМЕНЕНИЕ: ИСПРАВЛЕНА ГЕНЕРАЦИЯ КАРТИНОК ---

export const generateAiImage = async (prompt: string, size: "1K" | "2K" | "4K" = "1K"): Promise<string | null> => {
  try {
    // Используем тот же способ получения ключа, что и в рабочем квизе
    const apiKey = process.env.API_KEY;
    
    // Если ключа нет - сразу говорим об этом в консоль
    if (!apiKey) {
        console.error("API Key is undefined inside generateAiImage");
        return null;
    }

    const ai = new GoogleGenAI({ apiKey });

    // ВАЖНО: Используем правильный метод (generateImages) и быструю модель (Fast)
    ...
    // Добавил проверку: часто картинки не работают из-за отсутствия биллинга (даже на free tier)
    if (e.status === 403) {
        alert("Ошибка: Для генерации картинок API ключу нужен привязанный биллинг в Google Cloud.");
    }
    return null;
  }
};
